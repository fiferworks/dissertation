...
%this adds a header which says 'CHAPTER' to the toc
  ...
  \hypertarget{volatile-collection-trap-method-mm-vct}{%
  \subsubsection{Volatile Collection Trap Method \{mm-vct\}}\label{volatile-collection-trap-method-mm-vct}}

  The VCT method of collecting VOCs was based on a push-pull volatile collection method as illustrated in \emph{\ref{fig:vol-extract}}. Filtered air was provided by a 4-Port Positive Pressure Flow Out/Dual Y-Tubes \& Volatile Collection System (Sigma Scientific, Micanopy, FL USA) (2.0 \textbf{pressure?}). Air was blown through a teflon tube into a nylon oven bag ((\textbf{brand?})) that had been dried in an over at 50 °C overnight. The bag would be sealed over the end of a flowering rose cane from from Double Knock Out® roses. Bags were allowed to inflate in order to detect excessive leaks, if no large leaks were detected, the corner of the bag would be cut and a VCT would be inserted. Traps were 3.5'' with 28 ± 5 \si{\milli\gram} 80/100 mesh with HayeSep® Q adsorbent (Sigma Scientific, Micanopy, FL USA). Headspace volatiles were drawn through the filter by a vacuum line operating at (1.0 \textbf{pressure?}). Bags were left at positive pressure during volatile collection for 24 hours. The VOCs adsorbed inside the VCT filter were then eluted with 150 \si{\micro\litre}s of dichloromethane into a glass mandrel, then 5 \si{\micro\liter} of 1 \si{\micro\liter}/\si{\gram} Nonyl Acetate were added to each solution as an internal standard.

  \hypertarget{solid-phase-micro-extraction-method-mm-spme}{%
  \subsubsection{Solid Phase Micro Extraction Method \{mm-spme\}}\label{solid-phase-micro-extraction-method-mm-spme}}

  The SPME method was created to overcome a specific study limitation: after the detection of \emph{P. fructiphilus} in Florida, NFREC researchers were restricted from working with RRD-infected plants. In order to obtain volatile extractions from RRD-infected roses,
